...
echo -e "${green}#####################################${plain}"
# 检查omni服务是否已安装
if rc-status | grep -q omni; then
    # 如果服务已安装，检查其状态
service_status=$(rc-service omni status)
if echo "$service_status" | grep -q "starting" || echo "$service_status" | grep -q "started"; then
    echo -e "${green}omni运行中${plain}"
else
    echo -e "${red}omni未在运行${plain}"
fi
else
    echo -e "${red}omni服务未安装。${plain}"
fi
# Read user input
read -p "请输入数字: " choice
echo -e "${green}#####################################${plain}"
...
echo -e "${green}你选择了安装omni${plain}"
        ...
        # 下载编译好的omni (2024/1/24更新为Xiao的修改版omni)
        if  [ $# == 0 ] ;then
                last_version=$(curl -H "${auth_header}" -Ls "${url}/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
                if [[ ! -n "$last_version" ]]; then
                    echo -e "${red}检测 omni 版本失败，默认采用2024年1月28日版本${plain}"
                    last_version=v0.0.25
                fi
                echo -e "${green}检测到 omni 最新版本：${last_version}，开始安装${plain}"
                json=$(curl -H "${auth_header}" -Ls "${url}")
                filename="omni-linux-${arch}.zip"
                download_url=$(echo "${json}" | jq -r --arg filename "${filename}" '.[0].assets[] | select(.name == $filename) | .url')
                echo -e $download_url
                wget -q --header="${auth_header}" --header='Accept:application/octet-stream' -N --no-check-certificate -O /usr/local/omni/omni-linux.zip "${download_url}"
                if [[ $? -ne 0 ]]; then
                    echo -e "${red}下载 omni 失败，请确保你的服务器能够下载 Github 的文件${plain}"
                    exit 1
                fi
        fi
        unzip omni-linux.zip
        rm omni-linux.zip -f
        chmod +x omni
        mkdir /etc/omni/ -p
...
GEOIP_FILE="/etc/omni/geoip.dat"
GEOSITE_FILE="/etc/omni/geosite.dat"
...
if [ "$GEOSITE_CHECKSUM" == "$GEOSITE_HASH" ]; then
    ...
    echo -e "${GEOSITE_FILE} 的哈希值与云端不匹配，能用就行，跳过验证。"
        ...
        # 创建软链接
        ln -s /usr/local/omni/omni /usr/bin/omni
        # 添加到系统服务
        cat > /etc/init.d/omni <<EOF
#!/sbin/openrc-run
...
start() {
    ebegin "Starting omni"
    start-stop-daemon --start --exec /usr/bin/omni server
    eend $?
}

stop() {
    ebegin "Stopping omni"
    start-stop-daemon --stop --exec /usr/bin/omni
    eend $?
}

restart() {
    ebegin "Restarting omni"
    start-stop-daemon --stop --exec /usr/bin/omni
    sleep 1
    start-stop-daemon --start --exec /usr/bin/omni server
    eend $?
...
EOF
        # 给权限
        chmod +x /etc/init.d/omni
        # 添加开机自启动
        rc-update add omni default

        echo "安装完成"
        # 提示是否自动生成配置文件
		read -p "检测到你为第一次安装omni，是否自动直接生成配置文件？(y/n): " if_generate		
    ...
    if [ "$core_type" = 1 ]; then
        core_config="[
        {
            ...
            \"Log\": {
                \"Level\": \"error\",
                \"ErrorPath\": \"/etc/omni/error.log\"
            },
            \"OutboundConfigPath\": \"/etc/omni/custom_outbound.json\",
            \"RouteConfigPath\": \"/etc/omni/route.json\"
        }]"
        ...
        core_config="[
        {
            ...
            \"OriginalPath\": \"/etc/omni/sing_origin.json\"
        }]"
		fi
		#根据核心类型生成node部分的配置
		if [ "$core_type" == "1" ]; then
...
"CertConfig": {
	...
	"CertFile": "/etc/omni/fullchain.cer",
	"KeyFile": "/etc/omni/cert.key",
	"Email": "omni@github.com",
	"Provider": "cloudflare",
	"DNSEnv": {
		"EnvName": "env1"
			  }
...
EOF
    ...
    cat <<EOF > /etc/pmni/custom_outbound.json
    [
        {
            "tag": "IPv4_out",
            "protocol": "freedom",
...
EOF
    ...
    cat <<EOF > /etc/omni/sing_origin.json
{
  "outbounds": [
    {
      "tag": "direct",
...
EOF
echo -e "${green}omni 配置文件生成完成,正在重新启动服务,您原先的配置被被分到config.json.bak${plain}"	
echo -e "${green}再次运行 bash AlpineOmni.sh${plain}"
rc-service omni restart
	;;
*)
echo -e "${red}不生成配置,退出脚本,再次运行 bash Alpineomni.sh${plain}"
exit 0
	esac
;;
2)

read -p "确定要卸载 pmni 吗? (y/n): " uninstall_choice

        if [[ "$uninstall_choice" == "y" || "$uninstall_choice" == "Y" ]]; then
            echo "{red}正在卸载 omni...{plain}"
            rc-service omni stop
            rc-update del omni
            rm /etc/init.d/omni
            rm /etc/omni/ -rf
            rm /usr/local/omni/ -rf
            echo "{green}卸载成功，如果你想删除此脚本，则退出脚本后运行 'rm /usr/bin/omni -f' 'rm AlpineOmni.sh -f' 进行删除。{plain}"
        else
...
EOF
    # 创建 custom_outbound.json 文件
    cat <<EOF > /etc/omni/custom_outbound.json
    [
        {
            "tag": "IPv4_out",
            "protocol": "freedom",
